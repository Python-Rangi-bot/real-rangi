...
class Music:
    ...
    @commands.command(pass_context=True, no_pm=True)
    async def P(self, ctx, *, song : str):
        """노래틀떄 p하고 유튜브 영상이름을 치거나 p하고 유튜브 링크를 올리면 노래를 틀수있느니라!(사운드클라우드 등등 다른 곳도 된느니라!)
        """
        state = self.get_voice_state(ctx.message.server)
        opts = {
            'default_search': 'auto',
    ...
    @commands.command(pass_context=True, no_pm=True)
    async def v(self, ctx, value : int):
        """볼륨 설정중이니라."""
        ...
        if state.is_playing():
            ...
            await self.bot.say('음악 소리를 {:.0%}로 줄였느니라 헤헤'.format(player.volume))
    @commands.command(pass_context=True, no_pm=True)
